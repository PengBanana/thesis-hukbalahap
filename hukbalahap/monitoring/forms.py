...
class RegisterPool(forms.ModelForm):
    ...
    pool_ip = forms.ModelChoiceField(queryset=ipaddress_ref.objects.only('ipaddress').order_by('-id')[:1], required=True, label=('IP address'),widget=forms.Select(attrs={'class': 'form-control','autocomplete':'off', 'id': 'pool_ip'}))
    # ipaddress_ref.objects.only('ipaddress')
    class Meta:
        model = Pool
